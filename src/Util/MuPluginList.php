...
class MuPluginList
{
    ...
    private function mupluginsPathsInDir(
        ...
        foreach ($files as $file) {
            ...
            /** @var non-falsy-string $filePath */
            $filePath = $this->filesystem->normalizePath($file->getRealPath());
            if ($config !== null && $this->isDropinPath($filePath, $config)) {
                continue;
            }
            if ($single || $this->isPluginFile($filePath)) {
                $paths[] = $filePath;
            }
        }
    ...
    private function dropinsList(Config $config): array
    {
        ...
        foreach ($dropins as $dropin) {
            ...
            if ($dropinPath !== false) {
                $this->dropins[] = $this->filesystem->normalizePath($dropinPath);
            }
        }
