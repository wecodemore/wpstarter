...
use WeCodeMore\WpStarter\Utils\LanguageListFetcher;
...
final class DropinsStep implements StepInterface
{
     ...
     * @return static
     */
    public static function instance(IO $io) {
    ...
    public function run(Paths $paths, $verbosity)
    {
        ...
        foreach ($this->dropins as $name => $url) {
            $this->validName(basename($name))
                ? $this->runStep([$name => $url], $paths, $overwrite, $verbosity)
     ...
     * @param int $verbosity
     */
    private function runStep($dropin, Paths $paths, OverwriteHelper $overwrite, $verbosity)
    {
        list($name, $url) = $dropin;

        $step = new DropinStep($name, $url, $this->io, $overwrite);
        if ($step->allowed($this->config, $paths)) {
            $step->run($paths, $verbosity);
